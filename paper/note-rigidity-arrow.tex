...
but is not sufficient; eigenvector delocalization completes the relation.}
...
We then pre-registered a quantitative test over 23 configurations spanning the
...
\textbf{Q-3 (control) --- consistent.} The deterministic chord points sit at
the rigid end of the scatter with strong complexity ($q=0.9$ is the most
rigid configuration in the entire dataset, $\Svar=0.40$).
...
$\Klate(\Svar)$ curve. Eigenvector delocalization is the additional variable
that completes the relation.}
\end{quote}
