...
apply it uniformly to six families: the thermodynamic Past Hypothesis, Penrose's
Weyl curvature hypothesis, Barbour--Koslowski--Mercati Janus/shape-complexity
dynamics, causal-set sequential growth, the second law of quantum complexity,
observer/coarse-graining (decoherence, records, the generalized second law), and
the algebraic positive generator of a half-sided modular inclusion. Under the
test, four families either inherit the direction from a boundary condition or
manufacture it from an embedded observer; only two --- Janus dynamics and the
algebraic positive generator --- genuinely ignite an arrow from structure, and we
argue they most plausibly meet on a spectral quantity. We support the analysis
...
rewriting~\cite{arrighi}, or unconfined dynamics whose typical solutions already
grow two-sidedly~\cite{lazarovici}, the last an explicit bridge toward the Janus
...
Ceyhan--Faulkner~\cite{ceyhan} tie the relevant positive generator to
ANEC/QNEC in quantum field theory. Here the arrow is a \emph{spectrum condition}
rather than a coarse-graining. The objection, applied to our own bet: the inputs
may already carry the direction --- the vacuum/KMS state is not neutral, the
spectrum condition $\Pgen\ge 0$ is usually an axiom, and choosing which side is
...
Only the Janus (Family~3) and algebraic (Family~6) candidates ignite without a
boundary condition. Are they one mechanism? The most promising bridge is
\emph{spectral}. Miyaji et al.~\cite{miyaji} show that the complexity arrow's
linear-growth-then-saturation follows from random-matrix level repulsion --- the
arrow is read off the \emph{spectrum} of the generator, not a thermodynamic
gradient; and the algebraic positive generator is itself a spectrum condition.
Leutheusser--Liu~\cite{leutheusser} exhibit an emergent half-sided modular
inclusion in the large-$N$ thermofield double whose inclusion \emph{is} the
emergence of a directed interior time, and Gesteau~\cite{gesteau} treats the
appearance of such inclusions as an arrow-of-time diagnostic tied to
ergodic-hierarchy strength. This suggests a testable identification:

\begin{quote}
Does the complexity/Krylov arrow (Family~5 observable) and a modular-positivity
diagnostic (Family~6 origin) track the \emph{same spectral quantity} in a
computable model?
\end{quote}
...
\paragraph{The clock/arrow/antiunitary triad (exact).} From one
...
modular conjugation $J$ with $J\Delta J=\Delta^{-1}$. The project's separately
discovered clock, coarse-graining arrow, and antiunitary time-reversal are thus
the three objects any von Neumann algebra with a state produces.

\paragraph{Ignition from the spectrum (simulation).} Tridiagonalizing the SYK
Liouvillian into its Krylov chain, the complexity $K(t)$ is the position on a
semi-infinite chain --- a positive operator whose growth is an arrow. In chaotic
SYK$_4$ the chain opens and $K$ climbs (sustained complexity growing with system
size); in free SYK$_2$ it localizes. The origin is spectral: level-spacing ratios
are random-matrix-like for SYK$_4$ and Poisson-like for SYK$_2$. Crucially, the
thermal (KMS) detailed-balance asymmetry holds to numerical precision in
...
complexity and records but no extractable positive-order net would leave
Families~3 and~6 distinct.

\section{Why the exact ignition cannot be finite}

The exact object at the heart of Family~6 --- a nontrivial positive generator
...
is type~I, modular flow is inner and periodic, and the trace obstruction
$\mathrm{tr}[D,\Pgen]=0$ forbids an exact nonzero $[D,\Pgen]=i\Pgen$. A genuine
finite/lattice half-sided modular inclusion with $\Pgen\ge 0$ does not exist, and
this appears to be a real obstruction rather than a gap in the literature. Our
...
Janus dynamics and the algebraic positive generator ignite an arrow from
structure, and the most credible bridge between them is spectral. We have shown,
